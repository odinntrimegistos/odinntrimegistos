...
export function MentoringSection() {
  ...
  return (
    <section id="mentoria" className="relative overflow-hidden bg-obsidian py-12 md:py-16">
      <div className="max-w-5xl mx-auto px-6 relative z-10">
        <AnimatedSection className="text-center mb-12">
          <p className="font-display text-gold tracking-[0.3em] uppercase text-xs mb-4">Mentoria</p>
          <h2 className="font-display text-4xl md:text-5xl text-bone mb-6 uppercase">{content.title}</h2>
          <EgyptianDivider className="mb-8" />
          {content.subtitle && <p className="text-sand/80 text-lg mb-2">{content.subtitle}</p>}
            ...
            <AnimatedCard key={i} className="bg-graphite/40 backdrop-blur-sm border border-sand/10 rounded-lg p-6 hover:border-gold/30 transition-all flex flex-col">
              ...
              <div className="mb-6">
                <div className="text-gold font-display text-3xl">
                  {program.priceFormatted || `R$ ${program.price}`}
